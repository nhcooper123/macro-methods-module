...
To perform ancestral state estimation under the equal rates model:
...
We'll come back to this later in the practical.
...
Let's compare the multirate Brownian and multi-peak OU models to the models (BM, OU and EB) we fit in the previous exercise on models of continuous trait evoution.
Biologically, we're trying to see whether the evolution of body size is best fit as a single process (BM, OU or EB) across the whole tree, or if social and solitary magical creatures show different modes of body size evolution.

If you've started a new session since the previous practical, we need to fit the three simpler models again as follows:
...
```
Now we can compute AIC weights.
